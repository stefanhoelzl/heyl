            ...
            loop {
                ...
                return match find_trailers(&buf[..])? {
                    ...
                    FindTrailers::Done(len) => Poll::Ready(match len {
                        // PATCH (heyl): emit trailers that were stashed by an
                        // earlier `FindTrailers::Trailer` before ending the
                        // stream.
                        //
                        // When a data frame and the trailer frame arrive in the
                        // buffer together, the Trailer arm stores the trailers
                        // and returns the data. The next poll sees an empty
                        // buffer and lands here, where `0 => None` ended the
                        // body without ever emitting them -- so tonic reports
                        // "missing grpc-status trailer" on a response that
                        // arrived intact. The Trailer arm already does this
